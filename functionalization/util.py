...
def write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4):
    ...
    try:
        # transpose -> 式の行と列の入れ替え
        # contiguous -> 転地後のメモリ上の整地
        ind_nz = torch.nonzero(prediction[:, :, 4]).transpose(0, 1).contiguous()
    except:
        return 0

    box_a = prediction.new(prediction.shape) # いじる前のやつを入れる？
    # 多分幅
    ...
    prediction[:, :, :4] = box_a[:, :, :4]  # 閾値カットしたもののリセット？
    ...
    for ind in range(batch_size):
        ...
        try:
            img_classesl = unique(image_pred_[:, -1])
        except:
            continue
        # WE will do NMS classwise
        for cls in img_classesl:
